...
def g3_uniform(xi,cs=(2.5,3.0,3.5)):
    N=arb(N0)
    lo0,V1,_=rho_data(xi,arb(0,0.01))
    # V1 over zone-1 t range is what mid_uniform used; here only |t|<=0.01+NT is needed, but reuse a safe V over |t|<=0.2
    pass
    a2lo=arb(lo0.lower())
    out={}
    for c in cs:
        NT=arb(c)/(N*a2lo-3*V1).sqrt()
        assert (0.01+NT).upper()<0.9
        xib=xi+arb(0,NT.upper()); tb=arb(0,(0.01+NT).upper())
