...
def mid_uniform(xi, cs=(2.5,3.0,3.5)):
    N=arb(N0)
    # zone-1 data on t-balls covering [0, t1+0.01] (symmetric in t: use |t| via both signs)
    K1=16; edges=[(t1+0.01)*i/K1 for i in range(K1+1)]
    rho_lo=[]; V1=arb(0); a2abs=None
    for i in range(K1):
        best=None
        for sgn in (1,-1):
            tb=sgn*arb((edges[i]+edges[i+1])/2,(edges[i+1]-edges[i])/2+0.01)
    ...
    lo0,_,_=rho_data(xi,arb(0,0.01))
    ...
    for i in range(M):
        ...
        idx=min(int(tt/((t1+0.01)/K1)),K1-1); kk=float(kap[idx].lower())
    ...
    I2=arb(0)
    for i in range(K2):
        tb=arb((e2[i]+e2[i+1])/2,(e2[i+1]-e2[i])/2+0.01)
        lo,C=gam_data(xi,tb)
        gam=lo/2                                 # G <= -N gam + C
    ...
    ex3=-arb(3)/2*(Tlo-arb(2)/3*(arb('1.25')+arb('0.6673')*N/(10-0.01)))
