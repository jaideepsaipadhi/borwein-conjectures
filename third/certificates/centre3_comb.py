...
for iL in range(L_FROM, L_TO):
    ...
    line = []; OFFb = (10*PI*(arb(Aup)/(2*PI)).sqrt()*arb('-6.3').exp()).upper(); OFF2 = (10*PI*(arb(Aup)/(2*PI)).sqrt()*arb(-6.66).exp()).upper()
    for s in range(5):
    ...
    for x in line:   # AUDIT2 D-a: the combination must be asserted in arb, not read off printed decimals
        assert (arb(x[1]) - arb(err.upper()) - arb(OFFb)) > 0, ('COMBINATION FAILS', Llo, x[0])
    res.append(line); print("COMB L=%.3f aup=%.5f err_up=%.6f OFF63=%.6f OFF666=%.6f minslack=%.6f cls=%d" % (Llo, float(arb(Aup).mid()), float(err.upper().mid()), float(OFFb.mid()), float(OFF2.mid()), min(float((x[1]-err.upper()).mid()) for x in line), min(range(5), key=lambda s: float((line[s][1]-err.upper()).mid()))), flush=True)
print('COMBINATION ASSERTED (slack > OFF, arb) on every box; N0=%d  boxes=%d  all classes certified on every box: %s' % (N0, NL, all(all(x[2] for x in l) for l in res)))
for s in range(5):
